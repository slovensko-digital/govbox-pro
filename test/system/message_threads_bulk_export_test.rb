...
class MessageThreadsBulkExportTest < ApplicationSystemTestCase
  ...
  test "responsive: date inputs usable at mobile width (375px)" do
    thread = message_threads(:fs_accountants_thread1)
    export = Export.create!(
      user: users(:accountants_basic),
      message_thread_ids: [thread.id],
    ...
    from_input = find("input[name='export[settings][delivered_at_from]']")
    to_input   = find("input[name='export[settings][delivered_at_to]']")
    assert from_input.visible?, "od date input should be visible on mobile"
    assert to_input.visible?,   "do date input should be visible on mobile"
  ...
  test "responsive: preview table scrolls horizontally at tablet width (768px)" do
    thread = message_threads(:fs_accountants_thread1)
    export = Export.create!(
      user: users(:accountants_basic),
      message_thread_ids: [thread.id],
    ...
    assert_selector "th", text: "Schránka"
    assert_selector "th", text: "Súbor"
  ...
  test "responsive: Subor column visible at desktop (1400px) with long Nazov" do
    thread = message_threads(:fs_accountants_thread1)
    thread.update!(title: "Very long thread name that could push the Súbor column off screen if widths are not fixed properly")
    export = Export.create!(
      user: users(:accountants_basic),
      message_thread_ids: [thread.id],
      settings: { "messages" => true, "default" => true }
    ...
    subor_th = find("th", text: "Súbor")
    assert subor_th.visible?, "Súbor column must be visible at 1400px with long thread name"

    subor_rect = page.evaluate_script("document.querySelector('th:last-of-type').getBoundingClientRect()")
    assert subor_rect["right"] <= 1400, "Súbor column should not overflow viewport"
    assert subor_rect["left"] >= 0, "Súbor column should be on-screen"
