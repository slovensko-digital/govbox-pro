...
class ApiTokenPublicKeyValidatorTest < ActiveSupport::TestCase
  ...
  def test_rejects_private_key
    ...
    assert_equal "Je povolený iba verejný kľúč, nie súkromný", validator.error_message
  end
  ...
  def test_rejects_512_bit_key
    ...
    validator = ApiTokenPublicKeyValidator.new(key.public_key.to_pem)
    assert_not validator.valid?
    assert_equal "Kľúč musí mať presne 2048 bitov. Aktuálny kľúč má 512 bitov", validator.error_message
  end
  ...
  def test_rejects_4096_bit_key
    ...
    validator = ApiTokenPublicKeyValidator.new(key.public_key.to_pem)
    assert_not validator.valid?
    assert_equal "Kľúč musí mať presne 2048 bitov. Aktuálny kľúč má 4096 bitov", validator.error_message
  end
