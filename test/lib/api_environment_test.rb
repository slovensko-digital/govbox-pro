...
class ApiEnvironmentTest < ActiveSupport::TestCase
  ...
  class SiteAdminTokenAuthenticatorTest < ApiEnvironmentTest
    ...
    test 'fail on token verification with different key' do
      assert_raises(JWT::DecodeError) do
        @api_environment.site_admin_token_authenticator.verify_token(generate_api_token(key_pair: OpenSSL::PKey::RSA.new(2048)))
      end
    end
  end
  ...
  class TenantTokenAuthenticatorTest < ApiEnvironmentTest
    ...
    test 'fails on token verification for tenant with different key' do
      tenant = Tenant.new(name: "Test tenant", feature_flags: [:api], api_token_public_key: OpenSSL::PKey::RSA.new(2048).public_key)
      tenant.save

      assert_raises(JWT::DecodeError) do
        @api_environment.tenant_token_authenticator.verify_token(generate_api_token(sub: tenant.id, key_pair: OpenSSL::PKey::RSA.new(2048)))
      end
    end
  end
end
