...
class Upvs::ServiceWithFormAllowRule < ApplicationRecord
  scope :matching_metadata, ->(form) { where("schema_url LIKE ?", "%#{form.metadata['posp_id']}/#{form.metadata['posp_version']}") }

  def self.all_institutions_with_template_support(template)
    ::Upvs::ServiceWithFormAllowRule.matching_metadata(template)
  end
end
