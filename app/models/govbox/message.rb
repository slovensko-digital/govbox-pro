...
class Govbox::Message < ApplicationRecord
  ...
  def self.create_message_with_thread!(govbox_message, queue_priority: :medium_priority)
    message = nil
    ...
    EventBus.publish(:message_thread_created, message.thread, queue_priority) if message.thread.previously_new_record?
    EventBus.publish(:message_created, message, queue_priority)
