module Fs
  class DownloadReceivedMessageJob < ApplicationJob
    def perform(fs_message_id, box:, api_connection: box.api_connection, fs_client: FsEnvironment.fs_client)
      raise unless box.is_a?(Fs::Box)
      return unless box.syncable?

      message = ActiveRecord::Base.transaction do
        fs_api = fs_client.api(api_connection: api_connection, box: box)

        raw_message = fs_api.fetch_received_message(fs_message_id)

        Fs::Message.create_inbox_message_with_thread!(raw_message, box: box)
      end

      raise 'Signatures not yet verified!' if message.metadata["fs_submission_verification_status"] && !message.metadata["fs_submission_verification_status"]['description']&.include?('Overenie platnosti podpisov podania bolo ukončené')
    end
  end
end
